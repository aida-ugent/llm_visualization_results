...
# Run ID: 20250403_164705_milk_supply_medium, Iteration: 11
...
z = np.polyfit(filtered_df['date_ordinal'], filtered_df['Volume_1'], 1)
p = np.poly1d(z)
trend_line = p(filtered_df['date_ordinal'])

# Calculate R-squared
y_mean = np.mean(filtered_df['Volume_1'])
ss_tot = sum((filtered_df['Volume_1'] - y_mean) ** 2)
ss_res = sum((filtered_df['Volume_1'] - trend_line) ** 2)
...
# Plot the trend line
plt.plot(filtered_df['Date'], trend_line, 'r--', linewidth=1.5, label=f'Trend (R² = {r_squared:.2f})')
