...
# Run ID: 20250403_164705_milk_supply_medium, Iteration: 6
...
for _, row in yearly_avg.iterrows():
    ...
    plt.plot([year_data['Date'].min(), year_data['Date'].max()], [avg, avg], 
             linestyle='--', color='#ff7f0e', alpha=0.7, linewidth=1)
    plt.text(year_data['Date'].max() + pd.Timedelta(days=15), avg, 
             f"{year} avg: {avg:.0f}", verticalalignment='center', fontsize=8, color='#ff7f0e')
...
max_point = filtered_df.loc[filtered_df['Volume_1'].idxmax()]
plt.annotate(f"Peak: {int(max_point['Volume_1']):,} tons\n{max_point['Date'].strftime('%b %Y')}", 
             xy=(max_point['Date'], max_point['Volume_1']),
             xytext=(max_point['Date'] - pd.Timedelta(days=60), max_point['Volume_1'] + 20000),
