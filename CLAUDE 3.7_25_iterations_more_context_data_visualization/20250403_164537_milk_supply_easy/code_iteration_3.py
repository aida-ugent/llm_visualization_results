# Automatically saved code from agent execution
# Run ID: 20250403_164537_milk_supply_easy, Iteration: 3

import pandas as pd
import matplotlib.pyplot as plt
import matplotlib.dates as mdates
import seaborn as sns

# Set the style for the plot
sns.set_style("whitegrid")
plt.figure(figsize=(12, 6))

# Load the data
data_path = '/Users/alexanderrogiers/ugent/llm_visualization/data/milk_production_1.csv'
df = pd.read_csv(data_path)

# Convert 'Periods' to datetime for better plotting
# First, let's create a proper date format
def convert_period_to_date(period):
    parts = period.split()
    if len(parts) == 2:  # Monthly data (e.g., "1995 January")
        year = int(parts[0])
        month_dict = {
            'January': 1, 'February': 2, 'March': 3, 'April': 4, 'May': 5, 'June': 6,
            'July': 7, 'August': 8, 'September': 9, 'October': 10, 'November': 11, 'December': 12
        }
        month = month_dict.get(parts[1], 1)
        return pd.Timestamp(year=year, month=month, day=15)  # Using middle of month
    else:  # Yearly data (e.g., "1995")
        return pd.Timestamp(year=int(parts[0]), month=7, day=1)  # Middle of year

# Apply the conversion
df['Date'] = df['Periods'].apply(convert_period_to_date)

# Sort by date to ensure chronological order
df = df.sort_values('Date')

# Plot the cheese production volume over time
plt.plot(df['Date'], df['Cheese_5'], color='#f9a825', linewidth=2.5)

# Add markers for yearly data points to improve readability
yearly_data = df[df['Periods'].str.match(r'^\d{4}$')]
if not yearly_data.empty:
    plt.scatter(yearly_data['Date'], yearly_data['Cheese_5'], color='#e65100', s=50, zorder=5)

# Format the x-axis to show years
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
plt.gca().xaxis.set_major_locator(mdates.YearLocator(2))  # Show every 2 years
plt.xticks(rotation=45)

# Add labels and title
plt.xlabel('Year', fontsize=12)
plt.ylabel('Cheese Production Volume', fontsize=12)
plt.title('Cheese Production Volume in the Netherlands (1995-present)', fontsize=14, fontweight='bold')

# Add grid for better readability
plt.grid(True, linestyle='--', alpha=0.7)

# Annotate the most recent data point
last_point = df.iloc[-1]
plt.annotate(f'Latest: {int(last_point["Cheese_5"]):,}',
             xy=(last_point['Date'], last_point['Cheese_5']),
             xytext=(10, 0),
             textcoords='offset points',
             fontsize=10,
             fontweight='bold')

# Enhance the plot with a light background to improve contrast
plt.gca().set_facecolor('#f8f9fa')

# Add a subtle border
plt.gca().spines['top'].set_visible(False)
plt.gca().spines['right'].set_visible(False)
plt.gca().spines['left'].set_color('#dddddd')
plt.gca().spines['bottom'].set_color('#dddddd')

# Tight layout to ensure everything fits
plt.tight_layout()

# Save the figure
plt.savefig('/Users/alexanderrogiers/ugent/llm_visualization/output/20250403_164537_milk_supply_easy/visualization.png', dpi=300, bbox_inches='tight')