...
# Run ID: 20250403_165156_caribbean_births_medium, Iteration: 12
...
# Add percentage information to the melted dataframe
for i, row in plot_data_melted.iterrows():
    gender = row['Gender']
    region = row['Region']
    region_idx = plot_data[plot_data['Region'] == region].index[0]
    if gender == 'Boys':
        plot_data_melted.loc[i, 'Percentage'] = plot_data.loc[region_idx, 'Boys_Percentage']
    else:
        plot_data_melted.loc[i, 'Percentage'] = plot_data.loc[region_idx, 'Girls_Percentage']
...
for i, p in enumerate(ax.patches):
    height = p.get_height()
    percentage = plot_data_melted.iloc[i]['Percentage']
...
for i, region in enumerate(plot_data['Region'].unique()):
    region_total = plot_data[plot_data['Region'] == region]['Total'].values[0]
    plt.text(i, -15, f'Total: {region_total:.0f}', ha='center', fontsize=14, fontweight='bold')
