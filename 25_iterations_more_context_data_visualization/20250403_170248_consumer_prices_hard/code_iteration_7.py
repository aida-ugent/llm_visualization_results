...
# Run ID: 20250403_170248_consumer_prices_hard, Iteration: 7
...
# Find periods with high inflation (e.g., > 4%)
high_inflation = annual_data[annual_data['YoY_Change'] > 4].copy()
if not high_inflation.empty:
    for _, row in high_inflation.iterrows():
        if pd.notna(row['YoY_Change']):
            ax2.annotate(f"{row['YoY_Change']:.1f}%",
