...
# Run ID: 20250403_172703_population_medium, Iteration: 13
...
# Add a table with growth rates
growth_data = []
for bg in sorted(growth_rates.keys()):
    growth_data.append([bg, f"{growth_rates[bg]:.1f}%"])

# Create a table at the bottom of the figure
ax_table = fig.add_axes([0.15, 0.01, 0.7, 0.06])
ax_table.axis('off')
table = ax_table.table(
    cellText=growth_data,
    colLabels=['Migration Background', 'Growth Rate (2000-2010)'],
    loc='center',
    cellLoc='center'
)
